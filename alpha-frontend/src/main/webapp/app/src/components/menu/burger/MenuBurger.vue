  ...
  <Teleport :id="'teleport-menu-burger-' + id" to="body">
    <MenuMobile>
      <ContainerSlot :direction="FlexDirectionEnum.COLUMN">
        ...
        <ButtonPath label-button="component.button.settings" :size="SizeEnum.MEDIUM" routing-path="Settings" aria-label-button="component.button.settings" @click="openAddTagModal"/>
        <ButtonPath id="api" routing-path="Api" label-button="menu.burger.button.api" :size="SizeEnum.MEDIUM" v-if="authStore.isAdmin" aria-label-button="component.button.api" @click="openAddTagModal"/>
        <ButtonAction id="logout-action" label-button="menu.burger.button.logout" :function-click="deconnect" @clicked="logoutMessage" :size="SizeEnum.MEDIUM" v-if="authStore.isAdmin" aria-label-button="component.button.logout" @click="openAddTagModal"/>
      </ContainerSlot>
    </MenuMobile>
  </Teleport>
</template>
